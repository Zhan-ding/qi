...
class StrategyFactory:
    ...
    def get_custom_init_strategy() -> Callable[['InitGameMessage'], List[PieceArg]]:
        """
        选手自定义初始化策略：打造【重装战士 + 劈地法师 + 敏捷游侠】铁三角阵容
        """
        def strategy(init_message: 'InitGameMessage') -> List[PieceArg]:
            board = init_message.board
            pid = init_message.id
            
            # 1. 自动计算本方一侧的出生点（优先选择前线或特定扫描顺序的可通行格子）
            if pid == 1:
                order = [(x, y) for y in range(3, 0, -1) for x in range(2, board.width - 2)]
            else:
                order = [(x, y) for y in range(board.height - 4, board.height) for x in range(board.width - 3, 2, -1)]
            
            # 调用文件自带的辅助私有函数分配不重叠的坐标
            positions = _allocate_init_positions(board, pid, init_message.piece_cnt, order)
            piece_args: List[PieceArg] = []
            
            # 2. 为 3 个棋子定制不同的职业属性与装备配置
            
            # 棋子 0：重装突击战神（力量22换取3点AP，手持短剑身穿重甲，顶在最前线）
            arg0 = PieceArg()
            arg0.strength = 22
            arg0.dexterity = 8
            arg0.intelligence = 0
            arg0.equip = Point(2, 3)  # 武器2(短剑) + 防具3(重甲)
            arg0.pos = positions[0] if len(positions) > 0 else Point(0, 0)
            piece_args.append(arg0)
            
            # 棋子 1：超远真伤法师（智力12换取1个法术位，法杖12格手长，打无视抗性的真实伤害）
            if init_message.piece_cnt > 1:
                arg1 = PieceArg()
                arg1.strength = 13
                arg1.dexterity = 5
                arg1.intelligence = 12  # 刚好卡在 1 个法术位的阈值
                arg1.equip = Point(4, 1)  # 武器4(法杖) + 防具1(轻甲) *规则强制搭配
                arg1.pos = positions[1] if len(positions) > 1 else Point(1, 0)
                piece_args.append(arg1)
                
            # 棋子 2：极限先手机动游侠（敏捷17极大压制先手权，手持长弓全图风筝）
            if init_message.piece_cnt > 2:
                arg2 = PieceArg()
                arg2.strength = 13
                arg2.dexterity = 17  # 高敏捷，拿高顺位先手机会
                arg2.intelligence = 0
                arg2.equip = Point(3, 2)  # 武器3(弓) + 防具2(中甲)
                arg2.pos = positions[2] if len(positions) > 2 else Point(2, 0)
                piece_args.append(arg2)
    ...
    def get_custom_action_strategy() -> Callable[[Environment], ActionSet]:
        """
        选手自定义行动策略：根据当前行动棋子的职业（战士/法师/游侠）进行状态机决策
        """
        def strategy(env: 'Environment') -> ActionSet:
            action = ActionSet()
            current_piece = env.current_piece
            ...
            # 1. 获取场上所有存活的敌方棋子
            enemies = [p for p in env.action_queue if p.team != current_piece.team and p.is_alive]
            if not enemies:
                return action
                
            # 2. 预先计算核心目标：最近的敌人与血量最低的敌人
            nearest_enemy = min(enemies, key=lambda e: StrategyFactory.calculate_distance(current_piece.position, e.position))
            lowest_hp_enemy = min(enemies, key=lambda e: e.health)
            dist_to_nearest = StrategyFactory.calculate_distance(current_piece.position, nearest_enemy.position)
            ...
            # 【职业 A：重装战士 (Warrior)】 -> 3AP 莽夫，贴脸肉搏
            if current_piece.type == "Warrior":
                # 移动：无脑向最近的敌人逼近
                if legal_moves:
                    best_move = min(legal_moves, key=lambda m: StrategyFactory.calculate_distance(m, nearest_enemy.position))
                    action.move = True
                    action.move_target = best_move
                
                # 攻击：只要在短剑射程内，直接物理输出
                if dist_to_nearest <= current_piece.attack_range:
                    action.attack = True
                    action.attack_context = AttackContext()
                    action.attack_context.attacker = current_piece
                    action.attack_context.target = nearest_enemy
                    action.attack_context.attackPosition = current_piece.position

            # 【职业 B：炮台法师 (Mage)】 -> 12格真伤拉扯，关键火球压血
            elif current_piece.type == "Mage":
                # 法术决策：法术位是一次性资源，如果火球在手，且最近敌人进入施法范围(4)内
                available_spells = env.get_available_spells(current_piece)
                fireball = next((s for s in available_spells if s.id == 1), None)
                
                # 如果最近的敌人血量较低（低于15），或者已经贴脸，果断砸火球
                if fireball and current_piece.spell_slots > 0 and dist_to_nearest <= fireball.range:
                    if nearest_enemy.health <= 15 or dist_to_nearest <= 2:
                        action.spell = True
                        action.spell_context = SpellContext()
                        action.spell_context.caster = current_piece
                        action.spell_context.spell = fireball
                        action.spell_context.target_area = Area(nearest_enemy.position.x, nearest_enemy.position.y, fireball.area_radius)
                
                # 移动决策：如果没有放法术，法师尽量与敌人保持在 8~10 格的超远距离，安全白嫖真伤
                if legal_moves and not action.spell:
                    best_move = min(legal_moves, key=lambda m: abs(StrategyFactory.calculate_distance(m, nearest_enemy.position) - 9))
                    action.move = True
                    action.move_target = best_move
                
                # 攻击决策：法杖拥有 12 格超远射程，只要能打到就疯狂点射真伤
                if dist_to_nearest <= current_piece.attack_range:
                    action.attack = True
                    action.attack_context = AttackContext()
                    action.attack_context.attacker = current_piece
            ...
            # 【职业 C：敏捷游侠 (Archer)】 -> 集火脆皮，远程风筝
            else:
                # 游侠优先集火场上血量最低的“软柿子”
                target_to_hunt = lowest_hp_enemy if lowest_hp_enemy.is_alive else nearest_enemy
                dist_to_hunt = StrategyFactory.calculate_distance(current_piece.position, target_to_hunt.position)
                
                # 移动决策：保持在 6~8 格左右（长弓射程 9）进行安全的远程拉扯
                if legal_moves:
                    best_move = min(legal_moves, key=lambda m: abs(StrategyFactory.calculate_distance(m, target_to_hunt.position) - 7))
                    action.move = True
                    action.move_target = best_move
                    
                # 攻击决策：射击集火目标
                if dist_to_hunt <= current_piece.attack_range:
                    action.attack = True
                    action.attack_context = AttackContext()
                    action.attack_context.attacker = current_piece
                    action.attack_context.target = target_to_hunt
                    action.attack_context.attackPosition = current_piece.position
